...
class HydrologyApi:
    ...
    def get_stations_close_to_with_measure(
        self, lat, lon, radius, measure: Measure, limit=100
    ):
        api_url = self.API_BASE_URL + "id/stations"
    ...
    def get_measure(self, measure: Measure, station_id: str, start=None):
        ...
        result = self.http.request(
            ...
            api_url,
            fields={}
            | ({"mineq-date": start.strftime("%Y-%m-%d")} if start is not None else {}),
        ).data.decode("utf-8")
        data = json.loads(result)
        return pd.DataFrame(data["items"])
    ...
    def _batch_request(self, api_url):
        ...
        while status in ("Pending", "InProgress"):
            ...
            request = self.http.request(
                "GET", api_url, headers={"Accept-Encoding": "gzip"}
            )
    ...
    def batch_get_measure_from_stations(self, measure: Measure, stations):
        data = pd.DataFrame()
        threads = [
            self.thread_pool.submit(self.batch_get_measure, measure, station_id)
    ...
    def get_filename(self, measure: Measure, river):
        return f"{river.lower().replace(' ', '_')}_{measure.value}_raw.csv"

    def update_dataframe(
        self, df: pd.DataFrame, measure: Measure, stations: pd.DataFrame
    ):
        for station_name, station_id in stations[["label", "notation"]].values:
            last = (
                df[df["station"] == station_name]["dateTime"].max()
                if len(df) > 0
                else None
            )
...
def process_hydrology_data(df):
    return (
        df[df["quality"].isin(["Good", "Unchecked", "Estimated"])]
